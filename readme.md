# z_ipc：同机通信快得像在同一个进程里“传纸条”

> **一句话定义：** z_ipc 是一个让同一台机器上不同进程、不同语言之间，能像在同一个进程里传数据一样快的 IPC 引擎——**延迟 < 1ms，吞吐 10,000+ 请求/秒**。
>
...
## 二、z_ipc 是什么？——进程间通信的“高速公路”

z_ipc 是 **zAPI 跨语言 RPC 框架的 IPC 通信引擎**。它干的事很简单：**让同一台机器上的多个进程，能用接近“零延迟”的方式交换数据。**
...
目前直接支持：C、C++、Python、Go、Rust、Java、C#、VB.NET、Pascal（Delphi/FPC）。

通过 HTTP 网关（ZAPI Bridge）支持：PHP、Node.js、浏览器 JavaScript。

**Q4：需要额外安装什么依赖吗？**

不需要。z_ipc 是 header-only 的 C++ 库，所有依赖都在编译时处理好了。你只需要下载动态库，扔进项目目录，完事。
...
| **权衡** | 仅限同机（跨机器请用 TCP 模式） |
| **替代方案** | UDS（更慢）、原始共享内存（更麻烦）、gRPC（更重） |
...
- **Linux**：执行 `build_on_linux.sh` 即可全自动构建，脚本会自动处理 Boost 依赖并编译生成 `libz_ipc.so`。
- **macOS / BSD**：暂未提供预编译包，但可通过 CMake 自行编译（需安装 Boost）。
